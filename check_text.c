...
int check_text(char *src, char* the_ip_port, struct access_ctl_info access_info)
{
	int  ovector[OVECCOUNT] = {0}; 
	int  rc;
	int record_log = 1;
	printf("call check_text\n");

	DL_FOREACH(config_head, config_elt) {
		if (!memcmp(config_elt->key_ipp, the_ip_port, 12)) {
			if(config_elt->if_custom) { 
				if ( ( rc = pcre_exec(config_elt->cus, NULL, src, strlen(src), 0, 0, ovector, OVECCOUNT)) != PCRE_ERROR_NOMATCH ) {
					record_log = access_judge(src, the_ip_port, access_info);
					break;
				}	
			}
			else {
...
{	
	...
	//strategy_info.reset_flag = 0;
	memset(&strategy_info, 0, sizeof(strategy_info_t));
...
//	int check_policy_dba_result = 0;
	...
	{	
		...
		{	
			...
			if (access_elt->acc_type == 7 && access_info.if_where == 0)
				check_where_need = 0;
...
//access_judge_result is 1 means through, 0 means cut.
	...
	if(!check_policy_dba( src, the_ip_port )) {
